...
`GTKApplication::createWindow` builds a `GtkApplicationWindow`, sets title and default size. Default child is a vertical `GtkBox` (chrome) holding an empty top `GtkBox` (menu slot, vexpand false) then a `GtkFixed` (content). `$asGrid` still uses `GtkGrid` as the window child. Menu bar appends into the top slot, never unparents the Fixed.
