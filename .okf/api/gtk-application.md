...
| `createWindow($name, $width, $height, ?WindowSurface &$window = null, bool $asGrid = false)` | `static` | Throws `OSApplicationException::windowAlreadyCreated($name)` when the name is already stored. Otherwise `gtk_application_window_new`, `gtk_window_set_title`, `gtk_window_set_default_size`. When `$asGrid` is true, child is `gtk_grid_new()` and that is `content_pointer`. Otherwise window child is a vertical `GtkBox` (`chrome_pointer`) holding an empty top `GtkBox` (`chrome_top_pointer`, hexpand, vexpand false) then a `GtkFixed` (`content_pointer`, hexpand+vexpand). Chrome and Fixed get CSS `min-width: 0px; min-height: 0px;` so the window can shrink. Constructs `GTKWindowSurface`, stores in `$this->windows` |
| `closeWindow($name, ?WindowSurface &$window = null)` | `void` | Inherited from Tubes base. `GTKWindowSurface::close` is a no-op when `close-request` already destroyed the widget; the name is dropped so `createWindow` can reuse it |
